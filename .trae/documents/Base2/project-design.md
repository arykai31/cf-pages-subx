...
│   │   └── useSubscription.ts # 订阅相关 Hook
│   ├── lib/                  # 通用逻辑
│   │   ├── api.ts            # Fetch 封装（调用 Workers API）
...
│ │ │ └── 002\_create\_subscriptions.sql
...
- 配置路由（使用Cloudflare Pages App Router）
...
- 创建用户API（Cloudflare Workers + Elysia.js）
...
1. **安装依赖**
   ...
   ```bash
   # 创建Bun项目
   bun create react@latest subscription-system --template react-ts
...
5. **配置路由**
   ...
   - `src/app/profile/page.tsx`（个人中心页）
...
2. **Cloudflare Workers配置**
   - 创建`src/workers/index.ts`作为Workers入口点
